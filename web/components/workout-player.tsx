...
export function WorkoutPlayer({ workout, userId }: { workout: WorkoutDay; userId: string | null }) {
  ...
  async function cancelSession() {
    if (!state || busy || !window.confirm('این جلسه لغو و دادهٔ فعال آن حذف شود؟')) return;
    setBusy(true);
    try {
  ...
  async function finishSession() {
    ...
    try {
      ...
      setMessage('ذخیره نهایی تمرین ناموفق بود؛ جلسه هنوز برای تلاش دوباره فعال است.');
    } finally {
      setBusy(false);
    }
  }
  ...
  if (loading) {
    return <main className="workout-player-frame"><section className="workout-player-card is-loading" aria-busy="true">در حال آماده‌سازی جلسه…</section></main>;
  }

  if (!state) {
    return <main className="workout-player-frame"><section className="workout-player-card"><h1>جلسه آماده نشد</h1><p>{message}</p><Link href="/workout">بازگشت به تمرین‌ها</Link></section></main>;
  }
    ...
    return (
      <main className="workout-player-frame">
        <section className="workout-player-card workout-finished">
          ...
          <p className="workout-player-kicker">جلسه ثبت شد</p>
          <h1>{workout.title}</h1>
          <div className="workout-finished__stats">
            ...
            <div><strong>{fa.format(rpe)}</strong><span>RPE</span></div>
          </div>
          <p>{userId ? 'ست‌ها و بازخورد جلسه با RLS در حساب ذخیره شدند.' : 'جلسه در تاریخچه محلی همین مرورگر ذخیره شد.'}</p>
          <Link className="workout-player-primary" href="/workout">بازگشت به تمرین‌ها</Link>
        </section>
      </main>
    );
  }
    ...
    return (
      <main className="workout-player-frame">
        <section className="workout-player-card workout-completion-card">
          <p className="workout-player-kicker">تمام ست‌ها کامل شدند</p>
          <h1>بازخورد جلسه</h1>
          <div className="workout-completion-summary">
            <div><span>ست ثبت‌شده</span><strong>{fa.format(completeSets)}</strong></div>
            <div><span>حجم کل</span><strong>{fa.format(Math.round(volume))} kg</strong></div>
          </div>

          <label htmlFor="workout-rpe">سختی ادراک‌شده <b>{fa.format(rpe)} / ۱۰</b></label>
          ...
          {painScale >= 4 ? <p className="workout-player-warning">درد متوسط یا بیشتر ثبت شده؛ این داده برای محدودکردن پیشنهادهای بعدی مهم است.</p> : null}
          <label htmlFor="workout-notes">یادداشت جلسه</label>
          <textarea id="workout-notes" maxLength={2000} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="خواب، خستگی، حرکت سخت یا نکتهٔ جلسه بعد" />
          {message ? <p className="workout-player-error" role="status">{message}</p> : null}
          <button className="workout-player-primary" type="button" disabled={busy} onClick={() => void finishSession()}>{busy ? 'در حال ذخیره…' : 'ثبت نهایی تمرین'}</button>
          <button className="workout-player-link-button" type="button" disabled={busy} onClick={() => void cancelSession()}>لغو جلسه</button>
        </section>
      </main>
    );
  }
    ...
    return (
      <main className="workout-player-frame">
        <section className="workout-player-card workout-rest-card">
          <p className="workout-player-kicker">استراحت بین ست‌ها</p>
          <strong>{fa.format(restRemaining)}</strong>
          <span>ثانیه</span>
          <p>ست بعدی: {workout.exercises[currentWorkoutPosition(workout, state)?.exerciseIndex ?? position.exerciseIndex]?.name}</p>
          <button className="workout-player-primary" type="button" onClick={() => setRestRemaining(0)}>رد کردن استراحت</button>
        </section>
      </main>
    );
  }
    ...
    <main className="workout-player-frame">
      <section className="workout-player-card">
        <header className="workout-player-header">
          <Link href="/workout">خروج و ادامه بعداً</Link>
          <div><span>{userId ? 'ذخیره در حساب' : 'ذخیره محلی'}</span><b>{fa.format(progress)}٪</b></div>
        </header>
        ...
        <div className="workout-player-heading">
          ...
          <p>{fa.format(currentExercise.sets)} ست × {currentExercise.reps} تکرار · استراحت {currentExercise.rest}</p>
        </div>

        <div className="workout-player-set-number">ست {fa.format(position.setIndex + 1)}</div>
        <div className="workout-player-inputs">
          <label>وزنه (kg)<input inputMode="decimal" type="number" min="0" max="10000" step="0.5" value={currentSet.weightKg} onChange={(event) => patchDraft({ weightKg: event.target.value })} /></label>
        ...
        {message ? <p className="workout-player-error" role="status">{message}</p> : null}
        <button className="workout-player-primary workout-player-save-set" type="button" disabled={busy || !currentSet.reps || !currentSet.weightKg} onClick={() => void saveCurrentSet()}>{busy ? 'در حال ذخیره…' : 'ثبت ست و ادامه'}</button>
        <button className="workout-player-link-button" type="button" disabled={busy} onClick={() => void cancelSession()}>لغو کامل این جلسه</button>
      </section>
    </main>
  );
}
