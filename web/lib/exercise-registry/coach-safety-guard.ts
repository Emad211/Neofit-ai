...
export function deterministicWorkoutSafetyResponse(
  ...
  if (abnormalPain >= 4) {
    ...
    return `${source} فعلاً تمرین را ادامه نده و قبل از برنامه‌ریزی مجدد، وضعیت درد و محدودیت‌ها را به‌روز کن. اگر درد ادامه دارد، شدید است یا نگران‌کننده است، با پزشک یا فیزیوتراپیست واجد صلاحیت ارزیابی انجام بده؛ من تشخیص پزشکی نمی‌دهم.`;
  }
