...
def main() -> int:
    ...
    parser.add_argument("--expected-rows", type=int, default=11)
    parser.add_argument(
        "--output-format",
        default="ifkb-stage2-recovered-historical-assets",
    )
    ...
    if len(rows) != args.expected_rows:
        raise ValueError(
            f"Expected {args.expected_rows} recovery rows; found {len(rows)}"
        )
    ...
    for row in rows:
        ...
        if row["nutrition_gold_allowed"] != "no":
            raise ValueError(
                f"Internet asset cannot be Nutrition Gold: {row['candidate_id']}"
            )
        if row["automatic_identity_approval_allowed"] != "no":
            raise ValueError(
                f"Internet asset cannot self-approve identity: {row['candidate_id']}"
            )
        ...
        if digest != row["local_sha256"]:
            raise ValueError(
                f"SHA-256 mismatch for {row['candidate_id']}: "
                f"{digest} != {row['local_sha256']}"
            )
    ...
    summary = {
        ...
        "sourceManifest": args.manifest.name,
        "recoveredAssetRows": len(recovered),
        "coveredClasses": len({row["canon_id"] for row in recovered}),
        "roleCounts": dict(
            sorted(Counter(row["image_role"] for row in recovered).items())
        ),
