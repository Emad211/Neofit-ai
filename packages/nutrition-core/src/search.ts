...
function modifierTags(modifier: QueryModifier): readonly string[] {
  switch (modifier) {
    case 'without_yolk': return ['without_yolk', 'egg_white'];
    case 'egg_white': return ['egg_white', 'without_yolk'];
    case 'low_fat': return ['low_fat'];
    case 'grilled': return ['grilled', 'kebab'];
    case 'boiled': return ['boiled', 'poached'];
    case 'fried': return ['fried', 'breaded'];
    case 'without_added_fat': return ['without_added_fat', 'no_added_fat'];
    case 'with_oil': return ['with_oil', 'added_oil'];
    case 'skinless': return ['skinless'];
  }
}
...
function scoreVariant(
  ...
  if (query.length > 0) {
    if (candidateNames.includes(query)) {
      ...
      const overlap = Math.max(...candidateNames.map((name) => tokenOverlapScore(parsed.tokens, name)));
      score += overlap * 60;
      if (overlap > 0) reasons.push(`token_overlap:${overlap.toFixed(2)}`);
    }
  }
...
export function searchFoodDocuments(
  ...
  return documents
    .flatMap((document) => document.variants.map((variant) => scoreVariant(parsed, document.concept, variant)))
