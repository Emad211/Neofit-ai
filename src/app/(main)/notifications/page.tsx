...
export default function NotificationsPage() {
  ...
  React.useEffect(() => {
    if (isHydrated) window.localStorage.setItem(STORAGE_KEY, JSON.stringify(notices));
  }, [isHydrated, notices]);
    ...
    <main dir="rtl" className="min-h-screen bg-gradient-to-b from-primary/5 via-background to-background p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-3xl py-4">
        <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div><div className="flex items-center gap-2"><Bell className="h-6 w-6 text-primary" /><h1 className="text-3xl font-black">اعلان‌ها</h1></div><p className="mt-2 text-muted-foreground">{unread ? `${unread} اعلان خوانده‌نشده` : "همه اعلان‌ها خوانده شده‌اند"}</p></div>
          <Button variant="outline" onClick={() => setNotices((current) => current.map((notice) => ({ ...notice, read: true })))} disabled={!unread}><CheckCheck className="ml-2 h-4 w-4" />خواندن همه</Button>
        </header>
              ...
              return (
                <Card key={notice.id} className={cn("transition", !notice.read && "border-primary/35 bg-primary/5")}>
                  <CardContent className="flex items-start gap-4 p-4">
                    <button type="button" onClick={() => setNotices((current) => current.map((item) => item.id === notice.id ? { ...item, read: true } : item))} className="flex flex-1 items-start gap-4 text-right">
                    ...
                    <Button size="icon" variant="ghost" onClick={() => setNotices((current) => current.filter((item) => item.id !== notice.id))} aria-label={`حذف ${notice.title}`}><Trash2 className="h-4 w-4" /></Button>
                  </CardContent>
                </Card>
              );
            })}
