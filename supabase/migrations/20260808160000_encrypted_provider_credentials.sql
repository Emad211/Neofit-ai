create table public.encrypted_provider_credentials (
  ...
  constraint encrypted_provider_credentials_provider_valid
    check (provider in ('google', 'avalai')),
  constraint encrypted_provider_credentials_key_version_positive
    check (key_version > 0),
  constraint encrypted_provider_credentials_key_hint_length
    check (char_length(key_hint) between 4 and 12),
  constraint encrypted_provider_credentials_model_id_length
    check (char_length(btrim(model_id)) between 1 and 120),
  constraint encrypted_provider_credentials_status_valid
    check (status in ('active', 'invalid')),
  constraint encrypted_provider_credentials_ciphertext_length
    check (char_length(ciphertext) between 8 and 16384),
  constraint encrypted_provider_credentials_iv_length
    check (char_length(iv) between 8 and 128),
  constraint encrypted_provider_credentials_auth_tag_length
    check (char_length(auth_tag) between 8 and 128),
  constraint encrypted_provider_credentials_failure_code_length
    check (last_failure_code is null or char_length(last_failure_code) between 1 and 120),
  constraint encrypted_provider_credentials_user_provider_unique
...
comment on column public.encrypted_provider_credentials.provider is
  'AI provider identifier. Google is primary; AvalAI is fallback.';
comment on column public.encrypted_provider_credentials.cooldown_until is
  'Transient provider backoff boundary. The router skips a provider until this timestamp.';
